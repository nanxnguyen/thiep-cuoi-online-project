...
export function RsvpSection({ content, slug, preview, guestName }: { content: Content; slug?: string; preview: boolean; guestName: string }) {
  const { rsvp } = content;
  ...
  return (
    <section className="inv-section inv-rsvp" aria-labelledby="inv-rsvp-h">
      <Reveal>
        ...
        <RsvpForm slug={slug} preview={preview} guestName={guestName} questions={rsvp.questions} />
      </Reveal>
    </section>
  );
}
