...
Repo FE: `/Users/nguyenanhnhut/Desktop/Projects/thiep-cuoi-online-project` (nhánh `feat/invitation-core-phase1`, có commit nền `6a18590 phase 1`; working tree còn vài file chưa commit, xem mục 3 "Lệnh commit"). Repo BE: `/Users/nguyenanhnhut/Desktop/Projects/Thiep-cuoi-online-backend` (đã `git init -b main`, **vẫn 0 commit**). Cách chạy: `README.md` (FE) và `CLAUDE.md` (BE). Cổng: FE dev 3000, FE bản build `next start` 3001, BE 8090 (8080 bị stack Ecomerce chiếm), Postgres 5433, actuator 8081.
...
| Git: commit/branch, `git init` BE, xoá `dist/` + `scripts/site-smoke.test.mjs` | **FE ĐÃ COMMIT (chủ dự án tự chạy); BE CHƯA COMMIT** | 2026-09-22 xác nhận: `git log` FE có commit `6a18590 phase 1` trên nhánh `feat/invitation-core-phase1`, working tree đang có việc M9-M11/OG-image chưa commit (xem `git status`). BE đã `git init -b main` nhưng **0 commit** — chủ dự án tự chạy `cd ../Thiep-cuoi-online-backend && git add -A && git commit -m "feat: Spring Boot backend for invitations, RSVP, guestbook and media"`, hoặc cho phép `git commit` trong permissions |
